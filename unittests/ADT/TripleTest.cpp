...
TEST(TripleTest, ParsedIDs) {
  ...
  T = Triple("riscvxvec32-unknown-unknown");
  EXPECT_EQ(Triple::riscvxvec32, T.getArch());
  EXPECT_EQ(Triple::UnknownVendor, T.getVendor());
  EXPECT_EQ(Triple::UnknownOS, T.getOS());
  EXPECT_EQ(Triple::UnknownEnvironment, T.getEnvironment());

  T = Triple("riscvxvec64-unknown-linux");
  EXPECT_EQ(Triple::riscvxvec64, T.getArch());
  EXPECT_EQ(Triple::UnknownVendor, T.getVendor());
  EXPECT_EQ(Triple::Linux, T.getOS());
  EXPECT_EQ(Triple::UnknownEnvironment, T.getEnvironment());

  T = Triple("riscvxvec64-unknown-freebsd");
  EXPECT_EQ(Triple::riscvxvec64, T.getArch());
  EXPECT_EQ(Triple::UnknownVendor, T.getVendor());
  EXPECT_EQ(Triple::FreeBSD, T.getOS());
  EXPECT_EQ(Triple::UnknownEnvironment, T.getEnvironment());
...
TEST(TripleTest, BitWidthPredicates) {
  ...
  T.setArch(Triple::riscvxvec32);
  EXPECT_FALSE(T.isArch16Bit());
  EXPECT_TRUE(T.isArch32Bit());
  EXPECT_FALSE(T.isArch64Bit());

  T.setArch(Triple::riscvxvec64);
  EXPECT_FALSE(T.isArch16Bit());
  EXPECT_FALSE(T.isArch32Bit());
  EXPECT_TRUE(T.isArch64Bit());
}
...
TEST(TripleTest, BitWidthArchVariants) {
  ...
  T.setArch(Triple::riscvxvec32);
  EXPECT_EQ(Triple::riscvxvec32, T.get32BitArchVariant().getArch());
  EXPECT_EQ(Triple::riscvxvec64, T.get64BitArchVariant().getArch());
